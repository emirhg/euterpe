...
% loop until we have converged
    pS = gpuArray.zeros(M,L);
    pSi = gpuArray.zeros(M,N);
    converged = 0;
    ...
    while (converged == 0)
        ...
        % "E step"
        [pSi, alpha, beta, btpni] = gpuUpdown_tree(w, ES, POS, MU, SI, 1);
    ...
    if (iter >= maxIter)
        ...
        max(err2(:));
        max(err3(:));
        fprintf(1,'\t Class: %s \tError: %d\t%d\t%d\n', class(max(err1(:))), max(err1(:)), max(err2(:)), max(err3(:)));
...
ES = gpuArray.zeros(M,M^2,L);
POS = gpuArray.zeros(M,1);
MU = gpuArray.zeros(M,L);
SI = gpuArray.zeros(M,L);
